...
static DISCORD_QUERY_REC *create(const char *server_tag,
	...
	json_t *root = discord_get_channel_info(rec->server->tok, rec->name);
	rec->visible_name = g_strdup(json_string_value(json_object_get(json_array_get(json_object_get(root, "recipients"), 0), "username")));
